...
const CompetitorNearYou = () => {
  ...
  const handleSelectCompetitor = (competitor: BusinessProfile) => {
    const isSelected = store.selectedCompetitors.some(selected => selected.id === competitor.id);
    
    let updatedSelectedCompetitors;
    if (isSelected) {
      updatedSelectedCompetitors = store.selectedCompetitors.filter(selected => selected.id !== competitor.id);
    } else {
              ...
              <p className="text-gray-500">{competitor.address}</p>
              {
                <button
                  className={`mt-2 px-3 py-1 rounded text-sm font-medium ${
                    store.selectedCompetitors.some(selected => selected.id === competitor.id)
