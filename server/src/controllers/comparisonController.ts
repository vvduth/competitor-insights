...
export const suggestBusinessProfiles = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!profile) {
        res.status(400).json({ error: 'Business profiles are required for comparison' });
        return;
        }
...
export const compareBusinessProfiles = async (req: Request, res: Response) =>{
    try {
        ...
        if (!business || !competitors || competitors.length === 0) {
            res.status(400).json({ error: 'Business and at least one competitor are required for comparison' });
            return;
        }
        console.log("Comparing business profiles:", business, competitors);
        
        
        res.status(200).json("Comparison results will be here");
    } catch (error) {
        console.error('Error comparing business profiles:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
